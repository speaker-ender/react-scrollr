...
export const InViewComponent: React.FC<IInViewComponent> = props => {
    const [inViewRef, isInView] = useInViewState(props.threshold, props.untrackOnCallback, props.callback);

    return (
        <StyledInView transitionStyle={props.transitionStyle} ref={inViewRef} inView={isInView}>
            {props.children}
        </StyledInView>
    );
};
